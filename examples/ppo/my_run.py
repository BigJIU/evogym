...
def run_myppo(
        ...
        verbose=True):
    assert (structure == None) == (termination_condition is None) and (structure is None) == (saving_convention == None)
    ...
    print(f'{num_updates} {args.num_env_steps} {args.num_steps} {args.num_processes}\n')
        ...
        if j % args.log_interval == 0 and len(episode_rewards) > 1 and verbose:
            ...
            print(
                ...
                .format(j, total_num_steps,
                        ...
                        np.min(episode_rewards), np.max(episode_rewards), dist_entropy, value_loss,
                        action_loss))
